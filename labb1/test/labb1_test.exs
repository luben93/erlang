defmodule Labb1Test do
  ...
  test "start and get info" do
  	...
  	assert {:ok, [total: 5, taken: 2, occupied: 3]} == Labb1.get_info(pid)
    assert {:error,reason} = Labb1.start_link(-1,3)
 end

  test "taken and return " do 
  		{:ok,pid} = Labb1.start_link(5,2)
  		{:ok, [total: 5, taken: 2, occupied: 3]} = Labb1.get_info(pid)
  		...
  		assert {:ok, [total: 5, taken: 0, occupied: 5]} == Labb1.get_info(pid)		
  ...
  test "multiple clients" do 
  		...
  		{:ok, [total: 10, taken: 4, occupied: 6]} = Labb1.get_info(pid2)
